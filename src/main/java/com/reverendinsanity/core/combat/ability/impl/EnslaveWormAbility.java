...
// 奴虫蛊技能：奴道控制攻击
public class EnslaveWormAbility extends GuAbility {

    public EnslaveWormAbility() {
        super(GuRegistry.id("enslave_worm_gu"), 8f, 100, AbilityType.INSTANT);
    }

    @Override
    protected void onActivate(ServerPlayer player, Aperture aperture) {
        float efficiency = aperture.getEssenceGrade().getEfficiency();
        float pathBonus = 1.0f + aperture.getPathRealm(DaoPath.ENSLAVE).getTier() * 0.15f;
        double range = 6.0;
