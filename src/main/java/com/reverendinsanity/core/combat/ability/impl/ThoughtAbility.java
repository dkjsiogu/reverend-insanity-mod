...
// 思维蛊技能：精神攻击，造成无视护甲的伤害
public class ThoughtAbility extends GuAbility {

    public ThoughtAbility() {
        super(GuRegistry.id("thought_gu"), 8f, 100, AbilityType.INSTANT);
    }

    @Override
    protected void onActivate(ServerPlayer player, Aperture aperture) {
        float efficiency = aperture.getEssenceGrade().getEfficiency();
        float pathBonus = 1.0f + aperture.getPathRealm(DaoPath.WISDOM).getTier() * 0.15f;
        ...
        AABB hitBox = new AABB(eye, eye.add(look.scale(8.0))).inflate(1.0);
