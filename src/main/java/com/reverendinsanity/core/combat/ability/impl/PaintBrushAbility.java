...
// 画笔蛊技能：画道攻击
public class PaintBrushAbility extends GuAbility {

    public PaintBrushAbility() {
        super(GuRegistry.id("paint_brush_gu"), 8f, 100, AbilityType.INSTANT);
    }

    @Override
    protected void onActivate(ServerPlayer player, Aperture aperture) {
        float efficiency = aperture.getEssenceGrade().getEfficiency();
        float pathBonus = 1.0f + aperture.getPathRealm(DaoPath.PAINT).getTier() * 0.15f;
