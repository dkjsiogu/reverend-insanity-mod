...
// 万象画蛊技能：画道大范围幻象攻击
public class MyriadPaintAbility extends GuAbility {

    public MyriadPaintAbility() {
        super(GuRegistry.id("myriad_paint_gu"), 22f, 400, AbilityType.INSTANT);
    }

    @Override
    protected void onActivate(ServerPlayer player, Aperture aperture) {
        float efficiency = aperture.getEssenceGrade().getEfficiency();
        float pathBonus = 1.0f + aperture.getPathRealm(DaoPath.PAINT).getTier() * 0.2f;
        double range = 10.0;
