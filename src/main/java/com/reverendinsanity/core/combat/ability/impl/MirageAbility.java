...
// 海市蛊技能：海市蜃楼——前方6格幻象攻击
public class MirageAbility extends GuAbility {

    public MirageAbility() {
        super(GuRegistry.id("mirage_gu"), 8f, 100, AbilityType.INSTANT);
    }

    @Override
    protected void onActivate(ServerPlayer player, Aperture aperture) {
        float efficiency = aperture.getEssenceGrade().getEfficiency();
        float pathBonus = 1.0f + aperture.getPathRealm(DaoPath.ILLUSION).getTier() * 0.15f;
