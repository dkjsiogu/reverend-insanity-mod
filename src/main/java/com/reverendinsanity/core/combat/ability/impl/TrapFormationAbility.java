...
// 困阵蛊技能：布置困阵束缚敌人
public class TrapFormationAbility extends GuAbility {

    public TrapFormationAbility() {
        super(GuRegistry.id("trap_formation_gu"), 10f, 120, AbilityType.INSTANT);
    }

    @Override
    protected void onActivate(ServerPlayer player, Aperture aperture) {
        float efficiency = aperture.getEssenceGrade().getEfficiency();
        float pathBonus = 1.0f + aperture.getPathRealm(DaoPath.FORMATION).getTier() * 0.15f;
        double range = 6.0;
