...
// 万军蛊技能：万军冲锋——10格范围AOE军阵攻击（二转）
public class ThousandArmyAbility extends GuAbility {

    public ThousandArmyAbility() {
        super(GuRegistry.id("thousand_army_gu"), 22f, 400, AbilityType.INSTANT);
    }
    ...
    protected void onActivate(ServerPlayer player, Aperture aperture) {
        ...
        for (LivingEntity target : targets) {
            ...
            Vec3 away = target.position().subtract(center).normalize().scale(0.8);
        ...
        player.level().playSound(null, player.getX(), player.getY(), player.getZ(),
            SoundEvents.TRIDENT_THUNDER.value(), SoundSource.PLAYERS, 1.5f, 0.6f);
    }
}
