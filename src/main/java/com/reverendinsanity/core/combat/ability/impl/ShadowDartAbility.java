...
// 影针蛊技能：射出暗影针刺
public class ShadowDartAbility extends GuAbility {

    public ShadowDartAbility() {
        super(GuRegistry.id("shadow_dart_gu"), 8f, 100, AbilityType.INSTANT);
    }

    @Override
    protected void onActivate(ServerPlayer player, Aperture aperture) {
        float efficiency = aperture.getEssenceGrade().getEfficiency();
        float pathBonus = 1.0f + aperture.getPathRealm(DaoPath.SHADOW).getTier() * 0.15f;
